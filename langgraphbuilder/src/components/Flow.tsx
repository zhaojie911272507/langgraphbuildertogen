          ...
          <button
            className={`py-2 px-3 rounded-md transition-colors duration-200 ${
              !initialOnboardingComplete
                ...
                : 'bg-[#076699] cursor-pointer hover:bg-[#06578a]'
            }`}
            onClick={() => initialOnboardingComplete && setCustomFormModalOpen(true)}
            disabled={!initialOnboardingComplete}
          >
            <div className='text-slate-100 font-medium text-center'>保存&发布</div>
          </button>
      ...
      <div ref={reactFlowWrapper} className='no-scrollbar no-select' style={{ width: '100vw', height: '100vh' }}>
        ...
        <div className='hidden sm:block'>
          ...
          <MuiModal
            ...
            open={customFormModalOpen}
          >
            <ModalDialog className='bg-white hidden sm:block absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[500px]'>
              <div className='flex flex-col p-6'>
                <div className='flex justify-between items-center mb-4'>
                  <h2 className='text-xl font-medium'>👋保存</h2>
                  <button
                    className='text-gray-400 hover:text-gray-600'
                    onClick={() => setCustomFormModalOpen(false)}
                  >
                    <X size={24} />
                  </button>
                </div>
                ...
                <div className='space-y-4'>
                  <div>
                    ...
                    <select
                      className='w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#076699] focus:border-[#076699]'
                      value={formData.selectValue}
                      onChange={(e) => setFormData({...formData, selectValue: e.target.value})}
                    >
                      <option value=''>Choose an option</option>
                      <option value='option1'>资金</option>
                      <option value='option2'>税务</option>
                  ...
                  <div className='flex justify-end space-x-3 pt-4'>
                    ...
                    <button
                      className='px-4 py-2 bg-[#076699] text-white rounded-md hover:bg-[#06578a]'
                      onClick={() => {
                        // 处理表单提交逻辑
                        alert('提交成功')
